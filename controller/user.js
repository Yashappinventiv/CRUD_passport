...
module.exports = {
    ...
    async loginuser(req, res) {
        try {
            ...
            if (user_exits.length > 0) {
                ...
                if (pass) {
                    let tokken = jwt.sign({ id: user_exits[0]._id, email: email }, "secretkeyUser");
    ...
    async buyproduct(req, res) {
        try {
            ...
            //  await update.save()
            let update = await userModel.updateOne({ _id: currentId }, { $push: { buys: proId } })
            console.log(update)
            res.status(200).json({
    ...
    async totalProduct(req, res) {
        try {
            ...
            let data = await userModel.aggregate([
                  { $match: { _id: mongoose.Schema.Types.ObjectId(currentId) } },
                {
                    $lookup: {
                        from: "products",
                        localField: "buys",
