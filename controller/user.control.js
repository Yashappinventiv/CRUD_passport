       ...
       console.log(".....", res.locals.userId)
        let data = await userModel.aggregate( [
            ...
            {$lookup : { from : "posts" , localField : "_id"  , foreignField : "userId" , as : "total" } },
              {$unwind : "$total"}     ,
              {$lookup : {from : "postactions" , let : { userId : "$_id"  ,  postId : "$total._id"} , 
                  pipeline : [
                      {$match  : { $expr :  { $and  : [ {$eq : ["$likes.userId" , "$$userId"]} , 
 ...
 async individualType(req,res){
    ...
    let data = await postModel.aggregate([
        {$match : {  type : req.params.type }} , 
         {$lookup  :  { from  : "postactions"   , let  : {postid : "$_id" , postdata : "$$ROOT"}   , 
            pipeline  : [
                {$match  : { $expr :  { $and  : [ 
                {$eq : ["$likes.userId" , currentId]} , 
                {$eq :  ["$likes.postId" ,  "$$postid" ]} ] } }} ,
        ...
        as : "like"
            ...
            ,
        {$project : {isLiked : {$cond : { if : { $gt : [ { $size : "$like" } , 0 ] } , then : "true" , else  : "false" }} , 
